...
import BackToLock from '../components/BackToLoc';
import backgroundMusic from '../audio/steps-full.wav';
import backgroundImage from '../images/fone2.webp';
import YellowCrystal from '../images/yellowCrystal.png';
import businessPhone from '../images/businessPhone.webp';

const SecBuisnessWindow = ({
  fname,
  fIncome,
  fOnUpgrade,
  fUpgradeCost,
  ...
  return (
    <div style={{ backgroundImage: `url(${businessPhone})` }} className="p-5 rounded-lg shadow-lg">
      <h3 className="text-2xl font-bold mb-4 text-black">{fname}</h3>
      <p className="text-xl mb-4 text-black">
        Income:{' '}
        <span className="font-bold">
          ${fConvertNumberToShortForm(fIncome.toFixed(2) * multiplier.toFixed(2))}
        </span>{' '}
        per second
      </p>
      <p className="text-xl mb-4 text-black">
        Your Level: <span className="font-bold">{fLevel}</span>
      </p>
      <button
...
const World2Page = () => {
  ...
  const goBack = [{ id: 999, name: 'First Location' }];
  ...
  const [yellowCrystal, setYellowCrystal] = useState(
    () => Number(localStorage.getItem('yellowCrystal')) || 1000
  );
  const [fBalance, setFBalance] = useState(
    () => Number(localStorage.getItem('fBalance')) || 100000000
  );
  const [fIncome, setFIncome] = useState(() => Number(localStorage.getItem('fIncome')) || 100);
  const [fUpgradeCost, setFUpgradeCost] = useState(
    () => Number(localStorage.getItem('fUpgradeCost')) || 1000
  );
  const [fUpgradeCount, setFUpgradeCount] = useState(
    () => Number(localStorage.getItem('fUpgradeCount')) || 0
  );
      ...
      >
        ...
        {isStoreOpen && (
          <div
            style={{
              ...
              overflowY: 'auto',
              width: '400px',
              backgroundImage: `url(${businessPhone})`,
            }}
          ...
          >
            ...
            <div className="flex items-center mb-4">
              <img src={YellowCrystal} alt="Green Crystal" className="inline-block h-6 w-6" />{' '}
              <span className="font-bold">{yellowCrystal.toFixed(4)}</span>
            </div>
            <button
              onClick={convertBalanceToYellowCrystal}
            ...
            {goBack.map(goBack => (
              <BackToLock key={goBack.id} id={goBack.id} name={goBack.name} />
            ))}
        ...
        <p className="text-xl mb-2 text-black">
          ...
          <span className="font-bold text-black">
            ${fConvertNumberToShortForm(fBalance.toFixed(2))}
          </span>
        </p>
